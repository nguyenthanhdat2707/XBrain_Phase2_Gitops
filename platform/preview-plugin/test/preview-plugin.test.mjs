...
const imageLock = {
  ...
  mainSha: "main-sha",
  images: {
    frontend: "ghcr.io/nguyenthanhdat2707/mini-book-hub-frontend@sha256:stablefrontend",
    "book-service": "ghcr.io/nguyenthanhdat2707/mini-book-hub-book-service@sha256:stablebookservice",
    "reader-service": "ghcr.io/nguyenthanhdat2707/mini-book-hub-reader-service@sha256:stablereaderservice",
    "order-service": "ghcr.io/nguyenthanhdat2707/mini-book-hub-order-service@sha256:stableorderservice"
  }
};
...
test("resolves book-service PR into order preview runtime with stable dependencies", async () => {
  const metadata = {
    ...
    candidateImages: {
      "book-service": "ghcr.io/nguyenthanhdat2707/mini-book-hub-book-service@sha256:candidatebookservice"
    },
    stableImageLock: "image-locks/main.json",
  ...
  const fetchImpl = mockFetch({
    ...
    "https://api.github.test/repos/org/gitops/contents/preview-metadata/pr-124.json?ref=main": githubFile(metadata),
    "https://api.github.test/repos/org/gitops/contents/image-locks/main.json?ref=main": githubFile(imageLock),
    "https://api.github.test/repos/org/app/issues/124/labels?per_page=100": [{ name: "preview-ready" }]
  });
...
test("skips metadata when PR does not have preview-ready label", async () => {
  const metadata = {
    ...
    candidateImages: {
      "reader-service": "ghcr.io/nguyenthanhdat2707/mini-book-hub-reader-service@sha256:candidatereaderservice"
    },
    stableImageLock: "image-locks/main.json",
    previewProfile: "order-service",
  ...
  const fetchImpl = mockFetch({
    ...
    "https://api.github.test/repos/org/gitops/contents/preview-metadata/pr-125.json?ref=main": githubFile(metadata),
    "https://api.github.test/repos/org/app/issues/125/labels?per_page=100": [{ name: "needs-work" }]
  });
